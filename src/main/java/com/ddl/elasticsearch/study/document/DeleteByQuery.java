...
public class DeleteByQuery extends ElasticsearchClientBase {
    ...
    public void testForDeleteByQuery() throws Exception {
        DeleteByQueryAction.INSTANCE.newRequestBuilder(client).filter(QueryBuilders.termQuery("user", "quanke.name")).source("_all").execute().actionGet();
        //DeleteByQueryAction.INSTANCE.newRequestBuilder(client).filter(QueryBuilders.termQuery("user", "quanke.name")).source("_all").execute().actionGet();
        //.source("_all")
    ...
    public void testForAsyncDeleteByQuery() throws Exception {
        ...
        DeleteByQueryAction.INSTANCE.newRequestBuilder(client)
                ...
                .source("twitter")//_index
                .execute(new ActionListener<BulkByScrollResponse>() {
                    @Override
                    public void onResponse(BulkByScrollResponse response) {
                        long deleted = response.getDeleted();
                        System.out.println("异步删除成功！" + deleted);//删除了 但是没打印信息。
                    }
